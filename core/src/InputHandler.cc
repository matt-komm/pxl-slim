...
int InputHandler::readObjects(Event* event) 
{
	...
	while (getChunkReader().getInputStream().good())
	{
		...
		if (obj)
		{
			...
			throw std::runtime_error(
					"InputHandler::readObjects(pxl::Event* event): unknown object in file: "
							+ id.toString());
...
Serializable* InputHandler::readNextObject() 
{
	...
	if (obj)
	{
		...
		throw std::runtime_error(
				"InputHandler::readNextObject(): unknown object in file: "
						+ id.toString());
	return obj;
}
...
{
	if (getChunkReader().nextBlock()
			&& getChunkReader().getInputStream().good())
	{
		Id id(getChunkReader().getInputStream());
...
bool InputHandler::readBasicContainerIf(BasicContainer* basicContainer,
...
bool InputHandler::readBlockIf(const std::string& blockInfo, skipMode doSkip)
{
	...
	{
		success = getChunkReader().nextBlock(doSkip, ChunkReader::evaluate,
				blockInfo);
	}
	return success;
}
...
template<class objecttype> bool InputHandler::readObject(objecttype* obj)
		
{
	if (getChunkReader().getInputStream().good())
	{
		...
		if (id != objecttype::getStaticTypeId())
			throw std::runtime_error(
					"InputHandler::readObject(objecttype* obj): unexpected object in file: "
							+ id.toString());
		obj->deserialize(getChunkReader().getInputStream());
		return true;
	}

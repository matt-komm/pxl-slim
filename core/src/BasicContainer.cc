...
void BasicContainer::init(const BasicContainer& original)
{
	...
	for (const_iterator iter = original._container.begin();
	...
	// redirect index:
	for (map_t::const_iterator iter = original._index.begin();
			iter != original._index.end(); ++iter)
	{
		...
		Serializable* pNew = 0;
		std::map<Id, Serializable*>::const_iterator found = copyHistory.find(
				pOld->getId());
...
{
	for (map_t::const_iterator iter = _index.begin(); iter != _index.end();
			iter++)
	{
...
//	std::map<Id, Serializable*> objIdMap;
	...
	for (size_t i = 0; i < size; ++i)
	{
		Id typeId(in);
		Serializable* object =
				dynamic_cast<Serializable*>(ObjectFactory::instance().create(
						typeId));
		if (object)
		{
...
//			objIdMap.insert(std::pair<Id, Serializable*>(object->getId(), object));
		...
		{
			std::string errorMessage = 	"BasicContainer::deserialize(const InputStream &in) : unknown object (TypeID: " + typeId.toString() + ") in container!";
	...
	for (size_t i = 0; i < size; ++i)
	{
		...
		for (size_t j = 0; j < _container.size(); ++j)
		{
			if (id == _container[j]->getId())
				_index.insert(
						std::pair<std::string, Serializable*>(name,
								_container[j]));
		}

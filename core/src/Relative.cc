...
Relative::~Relative()
{
	...
	if (_refObjectOwner) {
		PXL_LOG_ERROR << "Error in ~Relative(): Relative derivative must be deleted by ObjectOwner to guarantee safe object handling!";
	}
}
...
void Relative::linkMother(Relative* target)
{
	...
	{
		throw std::runtime_error("pxl::ObjectBase::linkDaughter(...): ERROR: mother and daughter have not the same object holder!");
	}
...
void Relative::linkDaughter(Relative* target)
{
	if (target->_refObjectOwner != this->_refObjectOwner)
	{
		throw std::runtime_error("pxl::ObjectBase::linkMother(...): ERROR: mother and daughter have not the same object holder!");
	}
...
void Relative::linkFlat(Relative* target)
{
	if (target->_refObjectOwner != this->_refObjectOwner)
	{
		throw std::runtime_error("pxl::ObjectBase::linkFlat(...): ERROR: potential relatives have not the same object holder!");
	}

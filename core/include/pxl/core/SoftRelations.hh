...
public:
	...
	/// and which is contained in these soft relations. If the name of the soft relation \p type
	/// is also passed, the first Serializable which has the soft relation of the according type is returned.
	...
	/// is also passed, the first Serializable which has the soft relation of the according type is returned.
	/// Returns 0 in case no match is found.
	Serializable* getFirst(const BasicContainer& container, const std::string& type = "") const;

	/// Fills all Serializables which are both contained in the object owner \p owner as well as in these soft
	/// relations with the passed name \p type into the passed vector of Serializable pointers. 
	...
	/// relations with the passed name \p type into the passed vector of Serializable pointers. 
	/// Returns the number of filled Serializables.
	int getSoftRelatives(std::vector<Serializable*>& vec, const BasicContainer& container, const std::string& type) const;
	...
	/// Fills all Serializables which are both contained in the  object owner \p owner as well as in these soft
	/// relations with the passed name \p type into the passed vector of objecttype pointers. 
	...
	/// Returns the number of filled objects.
	template <class objecttype>
	int getSoftRelativesOfType(std::vector<objecttype*>& vec, const BasicContainer& owner, const std::string& type) const;
	...
	/// relations with the passed name \p type. 
	...
	/// with the name \p type.
	void set(const Serializable* relative, const std::string& type);

	/// Removes all existing soft relations with name \p type 
